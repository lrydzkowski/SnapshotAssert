...
The library SHALL capture the calling test's source file path via `[CallerFilePath]` on the `Verify`/`VerifyJson` entry points and use its directory as the snapshot directory, matching Verify's placement of snapshots next to the test file.
